...
public class Puzzle {
	...
	public Puzzle(int[]sr,int[]sc,CI[] i,int[] b){
		SR=sr.clone();
		SC=sc.clone();
		C=(byte) sr.length;
		R=(byte) sc.length;
	...
	public int SC(int j){byte r=0;for(byte i=0;i<C;i++)if(s(i,j))r++;return r;}
	public int SR(int i){byte r=0;for(byte j=0;j<R;j++)if(s(i,j))r++;return r;}
	public int IC(int j){byte r=0;for(byte i=0;i<C;i++)if(i(i,j))r++;return r;}
	public int IR(int i){byte r=0;for(byte j=0;j<R;j++)if(i(i,j))r++;return r;}
