...
uses personals, dmsrc, ventesA, global, epersonals, econcepte, ecaixa,
  etorns, magatzem, reports, main, pressupostos, averies, cventesA;

procedure T_eventesA.Nou;
begin
     TipusVenta := tvVenta;
...
function T_eventesA.Abonar(id : integer) : TModalResult;
begin
...
function T_eventesA.NetejaReferencies : boolean;
var
     s : string;
     q : TADOQuery;
begin
     ...
     if _ventesA.v.FieldByName('REF_pressupost').AsInteger <> 0 then s := 'l pressupost #' + _ventesA.v.FieldByName('REF_pressupost').AsString;

     if _ventesA.v.FieldByName('REF_AVERIA').AsInteger <> 0
     then
     begin
          if s <> '' then s := s + ' i a ';
          s := s + ' l''averia #' + _ventesA.v.FieldByName('REF_AVERIA').AsString;
     end;
     ...
     begin
          ...
          begin
               ...
               if _ventesA.v.FieldByName('REF_pressupost').AsString <> '' then
               begin
                    q.SQL.Text := 'update pressupostos set ref_factura = 0,ref_factura_lin = '''' where id = ' + _ventesA.v.FieldByName('REF_pressupost').AsString;
                    q.ExecSQL;
                    _pressupostos.v.Refresh;
               end;

               if _ventesA.v.FieldByName('REF_AVERIA').AsString <> '' then
               begin
                    q.SQL.Text := 'update averies set ref_factura = 0,ref_factura_lin = '''' where id = ' + _ventesA.v.FieldByName('REF_AVERIA').AsString;
                    q.ExecSQL;
                    _averies.v.Refresh;
               end;

               q.SQL.Text := 'update ventesA set ref_averia = 0,ref_pressupost = 0 where id = ' + _ventesA.v.FieldByName('id').AsString;
               q.ExecSQL;
...
procedure T_eventesA.FormActivate(Sender: TObject);
begin
...
procedure T_eventesA.TrobaObsCli;
var
     q : TADOQuery;
begin
...
procedure T_eventesA.dNIF_CLISetInnerCaption(Sender: TObject);
begin
...
procedure T_eventesA.dTEL_CLISetInnerCaption(Sender: TObject);
const
...
procedure T_eventesA.ComprobaCoherenciaDadesClient;
var
     q : TADOQuery;
begin
...
procedure T_eventesA.dCP_CLISetInnerCaption(Sender: TObject);
begin
...
procedure T_eventesA.tNouClientReact(Sender: TObject);
begin
...
procedure T_eventesA.tEditarClientReact(Sender: TObject);
begin
...
procedure T_eventesA.vCDelete(Sender: TObject);
var
     r : TModalResult;
begin
...
procedure T_eventesA.vCReturn(Sender: TObject);
var
     r : TModalResult;
begin
     if not tInsertarConcepte.Visible then exit;
...
procedure T_eventesA.vCInsert(Sender: TObject);
var
     r : TModalResult;
begin
     if not tInsertarConcepte.Visible then exit;
...
procedure T_eventesA.Editarconcepte1Click(Sender: TObject);
begin
     vC.PushReturn;
end;

procedure T_eventesA.tEditarConcepteReact(Sender: TObject);
begin
     vC.PushReturn;
end;

procedure T_eventesA.Insertar1Click(Sender: TObject);
begin
     vC.PushInsert;
end;

procedure T_eventesA.tInsertarConcepteReact(Sender: TObject);
begin
     vC.PushInsert;
end;

procedure T_eventesA.Eliminarconcepte1Click(Sender: TObject);
begin
     vC.PushDelete;
end;

procedure T_eventesA.tEliminarConcepteReact(Sender: TObject);
begin
     vC.PushDelete;
end;

procedure T_eventesA.LiniaArticles;
var
     n : integer;
begin
     ql.Active   := false;
...
procedure T_eventesA.CalculaPreus;
var
     q : TADOQuery;
begin
...
procedure T_eventesA.dIVA_PValueToCaption(Sender: TObject);
begin
     CalculaPreus;
end;

procedure T_eventesA.tIVAReact(Sender: TObject);
begin
     CalculaPreus;
end;

procedure T_eventesA.vCKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
     case Key of
...
procedure T_eventesA.UDBAlterFormCreateTempDetailTables(
  sender: TObject; Id: String);
var
     q : TADOQuery;
begin
     // es crea una taula nova amb els conceptes de c_ventesA
     ...
     q.Active := false;
     q.SQL.Text := 'insert into c_conceptes_tmp select c_ventesA.* from c_ventesA in ''' + dm.CR['database'] + ''' where (id_ref = ' + id + ')';
     q.ExecSQL;
...
procedure T_eventesA.UDBAlterFormDropTempDetailTables(sender: TObject;
  Id: String);
var
     q : TADOQuery;
begin
...
procedure T_eventesA.UDBAlterFormDeleteDetailValues(sender: TObject;
...
begin
     ...
     try
     q.Active := false;
     q.SQL.Text := 'select * from c_ventesA where id_ref = ' + id;
     q.Active := true;
     q.First;
...
procedure T_eventesA.UDBAlterFormInsertDetailValues(sender: TObject;
  Id: String);
var
     q : TADOQuery;
     q2 : TADOQuery;
...
begin
     ...
     try  q.Connection := dm.CT;
          ...
          q2.Connection := dm.CG;
          q2.Active := false;
          q2.SQL.Text := 'select max(id) as NewId from c_ventesA';
          q2.Active := true;

          idori := q2.FieldByName('NewId').AsInteger;

          q2.Active := false;
          q2.SQL.Text := 'select * from c_ventesA';
          q2.Active := true;
...
procedure T_eventesA.CarregaDadesVenta;
begin
     dDATA.Date   := date;
...
procedure T_eventesA.CarregaDadesAbonament;
var
     q : TADOQuery;
begin
     q := TADOQuery.Create(nil);
     ...
     q.SQL.Text := 'select * from ventesA where id = ' + inttostr(id_abonament);
     q.Active := true;
...
// n_serie
     ...
     q.SQL.Text := 'insert into c_conceptes_tmp select c_ventesA.* from c_ventesA in ''' + dm.CR['database'] + ''' where (id_ref = ' + inttostr(id_abonament) + ')';
     q.ExecSQL;
...
procedure T_eventesA.UDBAlterFormLoadDefValues(Sender: TObject);
begin
     case TipusVenta of
...
procedure T_eventesA.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
     case Key of
     27 : close;
...
procedure T_eventesA.UText3React(Sender: TObject);
begin
     vC.SetFocus;
end;

procedure T_eventesA.dPOB_CLISetInnerCaption(Sender: TObject);
begin
     lbGUIO.Visible := Trim(dCP_CLI.Caption) <> '';
     lbGUIO.Left    := dPOB_CLI.Left + dPOB_CLI.Width;
     dCP_CLI.Left   := lbGUIO.Left + lbGUIO.Width;
...
procedure T_eventesA.dDIR_CLISetInnerCaption(Sender: TObject);
const
     P : array [boolean] of string = ('','; ');
begin
     lbTEL.Visible := Trim(dTEL_CLI.Caption) <> '';
...
procedure T_eventesA.ColorBanda;
var
     c : TColor;
     s : string;
begin
     ...
     begin
          c := C_ENDEUTE;
          s := 'Amb deute pendent';
          if _ventesA.AmbCobraments then
     ...
     UDBAlterForm.NotAllowPost := _ventesA.AmbCobraments;
...
procedure T_eventesA.UDBAlterFormAfterLoadValues(Sender: TObject);
begin
     ColorBanda;
     CalculaPreus;
     ModificableONo;
...
procedure T_eventesA.ModificableONo;
var
     b : boolean;
begin
...
procedure T_eventesA.dTOTALSetInnerCaption(Sender: TObject);
begin
     if not (csLoading in ComponentState) and Visible
     then dPENDENT.Value := dTOTAL.Value - dA_COMPTE.Value;
end;

procedure T_eventesA.dPENDENTSetInnerCaption(Sender: TObject);
begin
     if not (csLoading in ComponentState) and Visible
     then dPAGADA.Value := (dPENDENT.Value = 0) and (dTOTAL.Value = 0);
end;

procedure T_eventesA.UDBAlterFormPostValues(Sender: TObject);
begin
...
procedure T_eventesA.ImprimirVenta;
begin
     case mDlg('Imprimir a full o a ticket?',['&Full','&Ticket','&No imprimir']) of
     mrYes : {full}   T_reports.Create(self).VeureVentesA;
     mrNo  : {ticket} _main.tImprTicket.React;
     mrCancel : ;
     end;
end;

procedure T_eventesA.CobrarVenta;
begin
     if _ventesA.v.FieldByName('PENDENT').AsFloat <> 0 then
     if _ventesA.v.FieldByName('PENDENT').AsFloat < 0 then
     begin
          ...
          T_ecaixa.Create(self).Abonament(_ventesA.v.FieldByName('id').AsInteger,lvA);
     end
     else
     case mDlg('Venta amb pendent. Cobrar venta?',mtInformation,[mbYes,mbNo]) of
     mrYes : if T_ecaixa.Create(self).Cobrar(_ventesA.v.FieldByName('id').AsInteger,lvA) <> mrOk then
               mDlg('Cobrament cancel·lat',mtInformation,[mbOk]);
     end;
end;

procedure T_eventesA.UDBAlterFormBeforePost(sender: TObject;
  var Stop: Boolean);
begin
     dHORA.Time := time;
...
procedure T_eventesA.FormCreate(Sender: TObject);
begin
...
procedure T_eventesA.dDESCOMPTE_PChangeValue(sender: TObject; V: Variant;
  var Cancel: Boolean);
begin
     if (UDBAlterForm.State = asLoading) or not visible then exit;
...
procedure T_eventesA.dDESCOMPTEChangeValue(sender: TObject; V: Variant;
  var Cancel: Boolean);
begin
     if (UDBAlterForm.State = asLoading) or not visible then exit;
...
procedure T_eventesA.dDESCOMPTE_PClick(Sender: TObject);
begin
     dDESCOMPTE_C.Value := true;
end;

procedure T_eventesA.dDESCOMPTEClick(Sender: TObject);
begin
     dDESCOMPTE_C.Value := false;
end;

procedure T_eventesA.Moureamunt1Click(Sender: TObject);
begin
     (PopupMenu.PopupComponent as TUDBView).CurrentUp;
end;

procedure T_eventesA.Moureavall1Click(Sender: TObject);
begin
     (PopupMenu.PopupComponent as TUDBView).CurrentDown;
end;

procedure T_eventesA.tMoureAmuntReact(Sender: TObject);
begin
     vC.CurrentUp;
end;

procedure T_eventesA.tMoureAvallReact(Sender: TObject);
begin
     vC.CurrentDown;
end;

procedure T_eventesA.dDATAChangeValue(sender: TObject; V: Variant;
  var Cancel: Boolean);
begin
...
procedure T_eventesA.dTIPUSChange(Sender: TObject);
var
     s : string;
begin
...
procedure T_eventesA.dNOM_CLIShowPickForm(sender: TObject; F: TCustomForm;
  U: TUDBView);
begin
...
procedure T_eventesA.FormDestroy(Sender: TObject);
begin
     ql.Free;
end;
