...
uses dmsrc, main, ADOdb, global, ecaixa, personals, gULIb, etorns, magatzem,
  averies, pressupostos, caixa, reports, tickprn, articles, eventesA;

procedure T_ventesA.IAfterScroll(dataset : TDataset);
begin
...
function T_ventesA.AmbCobraments : boolean;
var
     q : TADOQuery;
begin
     result := false;
     ...
     q.Active   := false;
     q.SQL.Text := 'select * from c_caixa where venta = ' + v.FieldByName('id').AsString + ' and linea_fact = ''A''';
     q.Active   := true;
...
procedure T_ventesA.vDelete(Sender: TObject);
begin
     _main.OnDelete(sender);
end;

procedure T_ventesA.vInsert(Sender: TObject);
begin
     _main.OnInsert(sender);
end;

procedure T_ventesA.vReturn(Sender: TObject);
begin
     _main.OnReturn(sender);
end;

procedure T_ventesA.NovaVentaClick(Sender: TObject);
begin
     v.PushInsert;
end;

procedure T_ventesA.Modificar1Click(Sender: TObject);
begin
     v.PushReturn;
end;

procedure T_ventesA.Eliminararticle1Click(Sender: TObject);
begin
     v.PushDelete;
end;

procedure T_ventesA.Aplicarfiltre1Click(Sender: TObject);
begin
     v.CallFilter;
end;

procedure T_ventesA.Eliminarfiltre1Click(Sender: TObject);
begin
     v.ClearFilter;
end;

procedure T_ventesA.Cobrar;
begin
     ...
     T_ecaixa.Create(self).Cobrar(v.FieldByName('id').AsInteger,lvA)
end;

procedure T_ventesA.Abonar;
var
     q : TADOQuery;
     s : string;
     id : integer;
begin
     ...
     if _ventesA.v.Data.Dataset.RecordCount = 0 then
     begin
     ...
     if not _ventesA.v.FieldByName('PAGADA').AsBoolean then
     begin
     ...
     begin
          q.SQL.Text := 'select * from ventesA where abonament_de = ' + v.FieldByName('id').AsString;
          q.Active := true;
          q.First;
     ...
     case T_eventesA.Create(Application).Abonar(id) of
     mrOk : begin
               q.Active := false;
               q.SQL.Text := 'select * from ventesA where id = ' + inttostr(id);
               q.Active := true;
               q.Edit;
...
procedure T_ventesA.FormCreate(Sender: TObject);
begin
     v.Data.Dataset.AfterScroll := IAfterScroll;
     _estilcolor.Apply(sender as TCustomForm,true);
end;

procedure T_ventesA.MostraMenu;
begin
     PopupMenu1.Popup(Mouse.CursorPos.x,Mouse.CursorPos.y);
end;

procedure T_ventesA.vCDblClick(Sender: TObject);
begin
     v.Locate('id',vc.FieldByName('id_ref').AsInteger,[]);
end;

procedure T_ventesA.ucKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
     case Key of
...
procedure T_ventesA.vGetFocus(Sender: TObject);
begin
     _main.PosaPanelDavant_(_main.paVentesA);
     _main.NMSEnabled;
     _main.IEnabled;     
end;

procedure T_ventesA.FormClose(Sender: TObject; var Action: TCloseAction);
begin
     _main.TancaLlista(sender);
end;

procedure T_ventesA.Abonar1Click(Sender: TObject);
begin
     Abonar;
end;

procedure T_ventesA.Cobrar1Click(Sender: TObject);
begin
     Cobrar;
end;

procedure T_ventesA.Clientrelacionat1Click(Sender: TObject);
begin
...
procedure T_ventesA.Averiarelacionada1Click(Sender: TObject);
begin
...
procedure T_ventesA.pressupostrelacionat1Click(Sender: TObject);
begin
...
procedure T_ventesA.Veurefactura1Click(Sender: TObject);
begin
     T_reports.Create(self).VeureVentesA;
end;

procedure T_ventesA.ImprimirTicket1Click(Sender: TObject);
begin
     tickprn.ImprimirTicket_A;
end;

procedure T_ventesA.PopupMenu1Popup(Sender: TObject);
begin
     Tornarllistaprvia1.Enabled := Length(_main.ALlistaPrevia) > 0;
end;

procedure T_ventesA.Tornarllistaprvia1Click(Sender: TObject);
begin
     _main.Tornarllistaprvia1Click(nil);
end;

procedure T_ventesA.Articlerelacionats1Click(Sender: TObject);
begin
...
procedure T_ventesA.Magatzemrelacionat1Click(Sender: TObject);
begin
...
procedure T_ventesA.vDrawRow(sender: TObject; R: TRect; F: TField;
...
procedure T_ventesA.MenuItem1Click(Sender: TObject);
begin
     vC.CallFilter;
end;

procedure T_ventesA.MenuItem2Click(Sender: TObject);
begin
     vC.ClearFilter;
end;

procedure T_ventesA.AplicarfiltreacolumnaF81Click(Sender: TObject);
begin
     v.CallColumnFilter;
end;

procedure T_ventesA.AplicarfiltreacolumnaF82Click(Sender: TObject);
begin
     vC.CallColumnFilter;
end;

procedure T_ventesA.Modificacipuntual1Click(Sender: TObject);
var
     d : TDateTime;
     s : string;
begin
...
procedure T_ventesA.Feraveria1Click(Sender: TObject);
var
     id : integer;
     idc : integer;
     q : TADOQuery;
...
begin
     ...
     q.SQL.Text := 'select * from c_ventesA where id_ref = ' + v.FieldByName('id').AsString + ' order by id asc';
     q.Active := true;
     q.First;
...
procedure T_ventesA.Veureabonaments1Click(Sender: TObject);
var
     id : string;
begin
