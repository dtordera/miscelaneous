...
public class SMTP extends SocketClient
{
     ...
     */
    protected Hashtable<String, String> _extensions_;

    /***
     * Is TLS already started.
     */
    protected boolean _isTLSStarted_ = false;
    ...
    public SMTP()
    {
        ...
        _extensions_ = new Hashtable<String, String>();
    }
    ...
    public boolean isExtensionSupported(String ext) {
    	return _extensions_ != null &&
    		_extensions_.get(ext.toUpperCase(Locale.ENGLISH)) != null;
    }

    public String getExtensionParameter(String ext) {
    	return _extensions_ == null ? null :
            (String)_extensions_.get(ext.toUpperCase(Locale.ENGLISH));
    }

    public boolean isAuthenticationSupported(String auth) {
    	if (_extensions_ == null)
            return false;

        String a = (String)_extensions_.get("AUTH");
        if (a == null)
            return false;
    ...
    protected void _parseExtensions_(List<String> lines) {
    	...
    	while (it.hasNext()) {
    		...
    		_extensions_.put(line.toUpperCase(Locale.ENGLISH), arg);
    	}
    }
    ...
    /***
     * A convenience method to send the SMTP EHLO command to the server,
     * receive the reply, and return the reply code.
     * <p>
     * @param hostname The hostname of the sender.
     * @return The reply code received from the server.
    ...
    {
        int ret = sendCommand(SMTPCommand.EHLO, hostname);
        _parseExtensions_(_replyLines);
        return ret;
    }

    protected void _startTLSAction_() throws IOException {
    	super._connectAction_();
    	_reader =
    		new BufferedReader(new InputStreamReader(_input_,
    				encoding));
    ...
    /***
     * A convenience method to send the SMTP STARTTLS command to the server,
     * receive the reply, and return the reply code.
     * <p>
     * @return The reply code received from the server.
     * @exception SMTPConnectionClosedException
     ...
     *      command or receiving the server reply.
     ***/
    public int starttls() throws IOException {
    	int ret = sendCommand(SMTPCommand.STARTTLS);
    	if (!SMTPReply.isPositiveCompletion(ret))
    		return ret;
