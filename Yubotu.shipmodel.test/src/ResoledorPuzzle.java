...
public class ResoledorPuzzle {
	...
	public boolean ComprobaCombinacio(Vaixell[] V){
		...
		return ComprobaFlota(V)&&CheckejaM(V);
	}
	
	public boolean ComprobaFlota(Vaixell[] V){
	...
	public void SolucionsPosibles(Vaixell[] check, Vaixell[] ori,int maxS){
		...
		for (Posicio p:ori[idx].ps.P) {
			...
			if (ComprobaCombinacio(check)) 
				...
				{							
					if (SolucioNoInclosa(check)&&CorrecteAmbM(check)){
	...
	public void FlotesPosibles(Vaixell[] check, Vaixell[] ori){	
	...
	public int Flotes(){
		...
		FlotesPosibles(new Vaixell[0],V);
	...
	public int Resol(int maxS){		
		...
		System.out.println(System.currentTimeMillis()-t);
/*		System.out.println(S.length + " solucio:");
