				...
				for (Index idx:I)
				{
					...
					{					
						...
						for(l=0;l<V[n].L;l++) {
							...
							det(T,V[n].L==1?K.B:l==0?K.O:l==V[n].L-1?K.E:K.X);
							T=T.E();
						} 
					...
					if (r) {
						...
						for(l=0;n<V[n].L;l++) {
							det(T.O(),K.A);
							det(T.E(),K.A);
							det(T,V[n].L==1?K.B:l==0?K.N:l==V[n].L-1?K.S:K.X);
							T=T.S();								
						} 
...
/*public boolean Coloca(Vaixell[] V,ArrayList<Index> I){
		...
		for (Index i:I)
			if (Coloca(V[0],i,)) 
				...
				return Coloca(Arrays.copyOfRange(V,1,V.length),I);
